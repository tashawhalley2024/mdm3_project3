...
**What it shows.** Two panels, both with the women's treatment index on the y-axis. **Left (composite secularism):** continuous x-axis, one country per dot, region-coloured; line is OLS with 95% CI shaded. **Right (apostasy):** violin + jittered strip — two violin bodies (no apostasy law / apostasy law) show the outcome distribution per level; per-country dots overlay with region-coloured jitter so individual observations remain readable. Both panels annotate slope (β), p-value, stars and sample size. No country labels on either panel; the panels are identified by region colour (left) and by violin-group geometry (right). A binary-coding footer sits under the right panel.

**Why it's here.** Before introducing fixed effects, panel data, or any controls, *what does the raw cross-sectional correlation look like?* The contrast is the visual headline for the deck, and the mixed geometry (continuous scatter vs binary violin) was chosen to reflect what each predictor actually is — a continuous composite score on the left, a 0/1 legal marker on the right.

**What the result means.** **Composite panel (left):** downward slope, β = **−0.096 (p = 0.028, \*\*)** in the 2020 cross-section under the T1 full-controls spec (rule of law, corruption, education, rurality, political stability, GDP). Countries higher on the composite score systematically lower on women's welfare. **Apostasy panel (right):** same direction, β = **−0.127 (p ≈ 0.002, \*\*\*)** under the same joint-GRI + controls spec. The apostasy-law violin body sits visibly below the no-apostasy-law violin in mean/median women's welfare; the apostasy-law group concentrates in MENA and Sub-Saharan Africa. The cross-section reading is consistent across both: more secular countries have better women's welfare, on average.

---

## 02_coefplot.png — Coefficient forest plot, T1 and T2

**What it shows.** Two panels with a shared y-axis: **Cross-sectional OLS (2020)** on the left, **Two-way fixed effects panel (2013–2022)** on the right. Each panel shows coefficients with 95% CIs for the two focal predictors (apostasy, composite), three hypothesis-context GRI sub-items (state religion, religious law, blasphemy), and the full `CONTROLS_GDP` block (rule of law, corruption, education, rurality, political stability, GDP), rendered in a visually-demoted grey band so the viewer reads them as covariates, not hypotheses. Suptitle: "Secularism and apostasy laws predict female treatment between countries but not within them." Subtitle: "Coefficient estimates with 95% CIs; GDP-controlled."
...
**What the result means.** **Composite:** T1 β = **−0.096 (p = 0.028, \*\*)**; T2 β = **+0.019 (p = 0.26, n.s.)**. Significant cross-country signal; within-country small, wrong-signed for the prior, and no longer significant under either clustered or Driscoll–Kraay SEs (post-winsorisation, T2_with_gdp_dk is p ≈ 0.12). The sign flip between tiers is the core empirical puzzle and is resolved at figure 03 (geometric) and figure 12 (formal). **Apostasy:** T1 β = **−0.127 (p < 0.01, \*\*\*)**; T2 β = −0.017 (p = 0.19 under clustered SEs, p ≈ 0.003 under DK). Consistent sign across tiers; the only sub-item whose within-country signal survives under DK. **Courts:** T1 ≈ 0 (n.s.); T2 β = −0.009 (n.s.). Null in both. The other GRI variables cluster around zero.

**Palette note.** The suptitle's "between but not within" framing matches the entity-clustered SEs that the figure draws (`T2_with_gdp` at line 308 of `run_plots.py`). If the tier is ever switched to DK, apostasy becomes p ≈ 0.003 and the suptitle must change; an inline comment in the code flags this dependency.

---

## 03_between_within.png — HERO FIGURE — between-country staircase with within-country arrows

**What it shows.** A single panel of the 2013–2022 composite panel (N = 186 countries with ≥3 non-null years). Country-level panel-means are plotted as small region-coloured dots. Behind them, thin grey arrows run from each country's 2013 (composite, women's welfare) point to its 2022 point — showing within-country movement. Overlaid on top, four large navy markers connected by a thick line trace the mean (composite, women's welfare) of each composite quartile Q1 (most secular) → Q4 (most religious). Anchor labels IRN, USA, NOR, IND, SAU, ZAF mark position on the panel-mean dots. A corner annotation quotes the formal T4 Mundlak numbers (β_between = −0.120, p = 0.014; β_within = +0.019, p = 0.24; ratio ≈ 6.3×, opposite signs) and points to figure 12 for the numerical companion.
...
**What the result means.** Headlines on each column read as prose, not stat-lines (Q7, Phoebe pass): "Zero sign flips across 166 country drops; K runs remain p < 0.05." **Composite (left):** zero sign flips across 166 single-country drops; zero runs fall below p < 0.05 under entity-clustered SEs (the within-country coefficient is small, wrong-signed for the prior, and marginal to begin with — the jackknife confirms that no single country is driving it in either direction). **Apostasy (right):** zero sign flips across 166 drops; 1 run remains p < 0.05 under entity-clustered SEs (many more are significant under DK — see 02_coefplot's T2_with_gdp_dk row). The bottom-row countries are the 5 most-influential individually, and none of them is enough to move either estimate across the sign-flip line.
...
**What it shows.** Two panels (composite | apostasy). Suptitle: "Results are robust across specifications: results are not driven by noise." Subtitle: "Composite and apostasy coefficients stable across nine specification variants." Each panel stacks the bivariate baseline and the main FE+GDP spec (highlighted band); the apostasy panel additionally shows seven sensitivity / subsample variants (CEDAW added, pre-COVID 2013–2019, changers-only, lag L1, lag L2, WEF GGG as alt DV, MENA-only). A small italic footer on the composite panel flags the asymmetry: *"Extended sensitivity specs fitted for apostasy only"* — a plotting-only disclosure; the upstream pipeline doesn't fit those tiers on the composite. Coefficients with 95% CIs, colour-coded by significance (two-tier in slide mode, four-tier in writeup mode).
...
**What the result means.** **Composite (left):** the within-country coefficient stays small and wrong-signed in both the bivariate baseline and the main spec. The cross-section coefficient (implicit in the bivariate baseline) remains strongly negative. **Apostasy (right):** Main, +CEDAW, Lag L1, Changers-only are all negative; Changers-only tightens to p<0.05 under entity-clustered SEs. Pre-COVID and Lag L2 stay negative, weaker. Only MENA-only approaches null (small N). Neither the composite's within-country null nor apostasy's within-country signal is a specification-fragile artefact.
...
**What it shows.** Forest plot with 11 rows: the composite-secularism coefficient (panel FE + GDP, 2013–2022) on each of the 10 WBL legal-rights group scores produced by `scoring.py` plus the Overall score at the top. A thick horizontal divider separates the Overall row from the components, with left-margin section labels "Overall" and "Components". Suptitle: "Within-country null holds on the overall WBL index." Subtitle: "Domain breakdown: Health and Entrepreneurship are marginal (p < 0.10); every other domain is null." Slide-side label simplifications (Q5): "Family & safety" → **Marriage**, "Economic rights" → **Entrepreneurship**, "Assets & Inheritance" → **Assets**. These are presentational narrowings; the underlying WBL taxonomy for "Marriage" also covers divorce and domestic-violence laws, and "Assets" includes inheritance.
...
**What the result means.** The overall WBL index shows β = **+0.019 (p = 0.26, n.s.)** — wrong-signed and null, consistent with the composite's within-country null at T2/T4. In the domain decomposition, **Health (β = +0.027, p = 0.064)** and **Entrepreneurship (β = +0.044, p = 0.050)** are marginal under p<0.10; every other domain is null. Neither marginal sub-domain is enough to rescue the composite's within-country narrative, and the within-country null is not an aggregation artefact — it holds at the most granular level we have.
...
**What it shows.** Two panels using the T4 Mundlak random-effects + fixed-effects hybrid (Bell & Jones 2015 framing). **Left:** the within-country effect for each predictor — how a *change* in the predictor within one country relates to a *change* in women's welfare. **Right:** the between-country effect — how a country's *average level* of the predictor relates to its *average level* of women's welfare. The composite row is highlighted. The between/within ratio is rendered as a small rounded-box inset in the bottom-right of the right panel (was previously a huge mid-figure callout; collapsed per Q14). A footer note flags that the x-axis scales differ between panels — that is the point, since the between panel carries the larger effect.
...
**What the result means.** **Composite within-effect:** β ≈ **+0.019 (p ≈ 0.24)** — small, wrong-signed for the prior, not significant. **Composite between-effect:** β ≈ **−0.120 (p = 0.014)** — negative, correctly signed, significant. The between-to-within ratio in magnitude is roughly **6.3×**, and the two estimates point in opposite directions. The composite secularism story is cross-national: countries at different points on the secularism spectrum look systematically different on women's welfare, and the structural gap between them is the dominant signal over the 2013–2022 window. This reframes the interpretation: the paper supports the claim that *more secular countries have better women's welfare*, but it does not provide positive evidence that *a country moving toward secularism will see women's welfare improve within a decade*.

**Apostasy in the Mundlak decomposition:** within ≈ **−0.017 (p ≈ 0.17** under entity-clustered SEs; **p ≈ 0.003 under DK)**, between ≈ **−0.146 (p < 0.001)**; ratio ≈ **8.6×** in magnitude, both correctly signed. Apostasy is the one sub-item whose within-country signal survives under DK SEs and preserves the expected direction. **Courts:** within ≈ −0.009 (n.s.), between small and wrong-signed (n.s. post-winsorisation). Retained as legacy null.
...
**What it shows.** Two panels for the long-difference regression, which collapses the panel to one observation per country by decade-endpoint subtraction (Δ = value in 2022 − value in 2013). **Left:** forest plot of the coefficient and 95% confidence interval across seven focal variants at 2013→2022 with GDP. Apostasy is greyed (9 changers, flagged invalid under the n_changers ≥ 10 threshold); courts carries a *low-power* warning with 19 changers. **Right:** the decade-change scatter for the headline equal-weight composite — one point per country, **uniform neutral fill** (no per-region colour — removes a second significance-style colour legend that was competing with the left forest's colouring), with OLS fit and 95% confidence band. The right-panel title is neutral ("Δ(WBL) vs Δ(Composite), 2013→2022") — the β/p/N stats for the composite row already appear on the left forest's Composite (eq-wt) row, so duplicating them in the title was redundant.
...
**What the result means.** Composite 2013→2022 with GDP: β = **+0.054 (p = 0.17, N = 162)** — wrong-signed for the prior, not significant. The within-country null therefore persists at the decade horizon rather than resolving at it: annual noise and WVS interpolation cannot be the binding explanation for the T2/T4 within null.
