...
The outcome dataset — **treatment-of-women indicators only**. All 32 numeric columns are
min-max normalised to **[0, 1]** (verified — no values outside this range). Missing values
are `NaN`, not imputed. **Higher always means better for women.**
Variables originally coded "higher = worse" have been inverted.
The one exception is `cedaw_ratified`, which is a raw binary flag (0/1) and is not normalised.

> **Religious composition variables have been moved** to
> `data/religion_composition/religion_composition_normalised.csv`.
> Join on `iso3 + year` to bring them back for analysis.
