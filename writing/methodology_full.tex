...
The remainder of the paper proceeds as follows. Section 2 describes the data and the construction of the composite secularism index. Section 3 sets out the five-tier empirical strategy. Section 4 reports the headline coefficients with the supporting figures. Section 5 presents the robustness battery. Section 6 discusses the substantive interpretation of the structural between-country result and the within-country null. Section 7 lists limitations. Section 8 concludes. Appendix A presents supporting robustness checks for the individual GRI sub-items and lists the checks we computed but excluded from the body, with reasons.
...
The institutional dimension aggregates six Pew Global Restrictions on Religion sub-items: state religion (\texttt{gri\_state\_religion\_norm}), government favouritism toward specific religions (\texttt{gri\_gov\_favour\_norm})\footnote{\texttt{gri\_gov\_favour\_norm} (Pew GRI Q2) was present in \texttt{data/predictors.csv} from the start of the project but was silently absent from \texttt{GRI\_PANEL\_COLS} in earlier analysis versions, with no deliberate exclusion documented. It was added to the institutional dimension as part of the Item 2 composite rebuild on 2026-04-15. The empirical sensitivity is reported in \texttt{results/gov\_favour\_sensitivity.csv} and amounts to a $-0.011$ shift in the T1 2020 with-GDP coefficient (main $-0.226$, excluded $-0.216$; $-4.7\%$ relative), a $+0.025$ shift in the T4 Mundlak between coefficient ($-0.278$ to $-0.252$; $-9.1\%$ relative), and a $-0.017$ shift in the T4 within coefficient ($+0.061$ to $+0.045$, $-27.6\%$ relative off a small base). The cross-country coefficient is robust to the inclusion; the within-country coefficient's magnitude is more sensitive to the extra GRI item, but both with and without \texttt{gri\_gov\_favour\_norm} the within-country estimate remains small, wrong-signed for the prior, and not reliably distinguishable from zero under entity-clustered inference.}, the role of religious law in the legal system (\texttt{gri\_religious\_law\_norm}), the formal role of religious courts (\texttt{gri\_religious\_courts\_norm}), blasphemy laws (\texttt{gri\_blasphemy\_norm}), and apostasy laws (\texttt{gri\_apostasy\_norm}). The attitudinal dimension uses one V-Dem indicator, \texttt{v2clrelig\_norm} (freedom to practise religion), which is sign-flipped before aggregation because its native orientation is opposite to the rest (higher V-Dem means more secular tolerance rather than more religion). The behavioural dimension uses four World Values Survey religious-intensity items: self-reported importance of religion (\texttt{wvs\_imprel\_norm}), importance of God (\texttt{wvs\_godimp\_norm}), belief in God (\texttt{wvs\_godbel\_norm}), and confidence in religious institutions (\texttt{wvs\_confch\_norm}).
...
We report the composite under two alternative dimension weightings as a sensitivity check on the equal-weight choice. \texttt{composite\_secularism\_instonly\_norm} drops the attitudinal and behavioural dimensions entirely and takes the z-score mean of the six institutional GRI items alone. \texttt{composite\_secularism\_covwt\_norm} weights each dimension by its panel-wide non-null row fraction, where the WVS behavioural dimension's coverage is computed on pre-interpolation wave years so the variant is informative even when the main composite is built on post-interpolation data. The covwt composite is still constructed from the post-interpolation WVS values; only the weight the behavioural dimension receives is reduced to reflect its real-coverage share. Readers should treat covwt as a blended composite with a small interpolator-exposed behavioural share, not as a WVS-real-only variant. On the 2057-row merged panel the raw dimension coverages are $0.90$ for institutional (6 GRI columns observed), $0.83$ for attitudinal (\texttt{v2clrelig} observed), and $0.05$ for behavioural (all four WVS columns observed in a real wave year). The coverage-weighted composite therefore places roughly $51\%$ weight on the institutional dimension, $46\%$ on attitudinal, and $3\%$ on behavioural, against the equal-weight share of $33\%$ on each. Table \ref{tab:weight-sensitivity} reports the headline coefficients under the three weightings.

\begin{table}[h!]
\centering
\caption{Composite secularism coefficients under three dimension weightings. Institutional-only drops the attitudinal and behavioural dimensions. Coverage-weighted downweights the behavioural dimension by its pre-interpolation panel coverage. All rows use the same estimation samples as the main composite.}
\label{tab:weight-sensitivity}
\begin{tabular}{lccc}
\toprule
Specification & Equal & Institutional-only & Coverage-weighted \\
\midrule
T1 2020 with GDP ($\hat\beta$, $p$)      & $-0.226$, $<0.001$  & $-0.170$, $<0.001$  & $-0.205$, $<0.001$ \\
T2 with GDP within ($\hat\beta$, $p$)    & $+0.061$, $0.07$    & $+0.007$, $0.64$    & $+0.038$, $0.20$   \\
T4 Mundlak within ($\hat\beta$, $p$)     & $+0.061$, $0.06$    & $+0.007$, $0.63$    & $+0.038$, $0.18$   \\
T4 Mundlak between ($\hat\beta$, $p$)    & $-0.278$, $<0.001$  & $-0.175$, $<0.001$  & $-0.236$, $<0.001$ \\
\bottomrule
\end{tabular}
\end{table}

The cross-sectional and between-country coefficients retain their magnitude and sign under every weighting, with the institutional-only variant producing the most conservative estimates (about $25\%$ smaller in absolute magnitude than equal-weight). The within-country wrong-signed positive estimate that the equal-weight composite carries ($+0.061$, marginally significant) collapses to essentially zero under the institutional-only weighting ($+0.007$) and halves under coverage-weighting ($+0.038$, not significant). The gradation has a clean read. Institutional-only is a pure GRI composite with neither attitudinal nor behavioural input; its within-country coefficient is essentially zero, confirming that the institutional dimension alone does not produce a within-country signal in the 2013--2022 panel. Coverage-weighting puts $46\%$ of the weight on \texttt{v2clrelig} (attitudinal) and only $3\%$ on WVS (behavioural), yet still produces a within-country point estimate of $+0.038$ -- larger than institutional-only but smaller than equal. The point estimate is not statistically distinguishable from zero (the $95\%$ confidence interval spans roughly $[-0.02, +0.10]$), so the data do not reject a reading in which the residual movement is WVS-driven even under covwt's heavy down-weighting; they also do not reject a reading in which \texttt{v2clrelig} carries a non-trivial share (\texttt{v2clrelig} is updated annually by V-Dem coders on qualitative grounds, so small year-over-year movement is real). What the three-way comparison does settle is the converse claim: the within-country positive sign is not robust across weightings, collapses under institutional-only, and attenuates under any re-weighting that reduces the WVS share. This points the same direction as the no-interpolation variant reported in \S\ref{sec:within-null}.
...
The five estimators are not parallel alternatives. They form an escalating identification strategy: each tier addresses a threat to validity that its predecessor cannot. Throughout, $W_{it}$ denotes the women's treatment index for country $i$ in year $t$, $S_{it}$ the composite secularism predictor, $\mathbf{X}_{it}$ the V-Dem governance controls and (where included) log GDP per capita, $\alpha_i$ time-invariant country effects, $\lambda_t$ year effects, and $\varepsilon_{it}$ an idiosyncratic error.
...
\subsection{Tier 5: Long-Difference Regression}

\subsubsection{Specification}

Tier 5 collapses the panel to a single observation per country by taking endpoint differences. For the main window we compute, for every country $i$ with both endpoints observed,
\begin{equation}
\Delta W_i = W_{i,2022} - W_{i,2013}, \quad \Delta S_i = S_{i,2022} - S_{i,2013}, \quad \Delta \mathbf{X}_i = \mathbf{X}_{i,2022} - \mathbf{X}_{i,2013},
\end{equation}
and estimate
\begin{equation}
\Delta W_i = \alpha + \beta\, \Delta S_i + \boldsymbol{\gamma}^{\prime}\, \Delta \mathbf{X}_i + u_i.
\end{equation}
Standard errors are HC3 robust. Clustering is not required because each country enters exactly once; HC3 is the appropriate heteroskedasticity adjustment for this cross-section of decade changes. We report the main 2013--2022 window, an alternative 2014--2022 window that lets countries with thinner 2013 focal coverage enter the sample, and two 5-year sub-windows (2013--2017 and 2017--2022) as within-decade robustness. Rows are flagged as invalid when fewer than ten countries actually change on the focal predictor over the window and flagged as low-power when the count is between ten and twenty-four.

\subsubsection{Motivation}

The TWFE estimator (Tier 2) and the Mundlak within component (Tier 4) both identify the within-country coefficient off annual movement. In a short panel with slow-moving institutional regressors, much of the annual movement is short-term noise; in the present data, the behavioural dimension of the composite is linearly interpolated between WVS waves, so year-to-year movement on that dimension is interpolator arithmetic rather than genuine attitudinal change. The long-difference removes annual variation from the identifying signal by using only endpoint observations. If the within-country null at Tiers 2 and 4 is driven by annual noise or by interpolator contamination, the long-difference would show a coefficient closer to the structural cross-country direction. If the null persists at the medium-run horizon, the annual-noise and interpolator explanations cannot account for it, and the within-country null is the more defensible reading of the evidence. Either outcome is informative, and the paper reports whichever the data produce.

\subsubsection{Literature Precedent}

Long-differencing is standard in comparative-development empirics when short annual panels cannot identify slow-moving institutional change. \citet{AcemogluJohnsonRobinsonYared2008} uses long-differences to estimate the effect of income on democracy across 25-year windows because annual panel variation is dominated by business-cycle movement. \citet{BesleyPersson2011} employ the same identification strategy for state-capacity and development outcomes, with the argument that within-country institutional change operates at decade-to-generation timescales rather than year-to-year. The present application inherits the same motivation: the composite secularism index is built from legal institutional indicators that move rarely within country and from survey-based indicators whose annual variation is partly artificial.

\section{Results}

\subsection{Headline Coefficients}

Table~\ref{tab:headline} reports the coefficient on the composite secularism predictor across the five tiers, with the equal-weight and PCA variants side by side. Apostasy and religious courts are reported as sub-item robustness entries. All coefficients are on the $[0, 1]$ welfare scale, so a coefficient of $-0.226$ corresponds to a $0.226$-point drop in the women's treatment index for a one-point increase in the (also $[0, 1]$-scaled) composite predictor.

\begin{table}[H]
\centering
\caption{Headline coefficients across the five tiers. T1 reports the with-GDP cross-section in 2020. T2 reports the with-GDP TWFE panel ($n = 1{,}648$). T5 reports the long-difference regression over 2013--2022 with GDP ($n = 163$, one observation per country). T4 reports the within and between components from the Mundlak correlated random-effects model ($n = 1{,}648$). Standard errors in parentheses. T3 system-GMM ($n = 1{,}316$) is reported in the main text but not in this table because it violates the Roodman bounds check. The composite (equal-weight) is the headline focal predictor; the composite (PCA) is a robustness variant. Apostasy is retained as the strongest individual sub-item robustness; religious courts is the legacy null predictor from earlier versions of this analysis. Source: \texttt{results/headline\_table.csv}.}
\label{tab:headline}
\begin{tabular}{lcccc}
\toprule
...
T5 long-diff (2013--2022, with GDP) & $+0.082$ ($0.076$) & $-0.015$ ($0.056$) & --\textsuperscript{a} & $-0.010$ ($0.013$)\textsuperscript{b} \\
...
\smallskip
\\
{\footnotesize \textsuperscript{$\dagger$} $p < 0.10$, \textsuperscript{**} $p < 0.05$, \textsuperscript{***} $p < 0.01$. \textsuperscript{a} Apostasy T5 row flagged invalid: only nine countries record any apostasy-law movement over 2013--2022, below the $n_{\text{changers}} \geq 10$ validity threshold. The underlying point estimate is $-0.042$ ($p = 0.17$, $N = 163$, direction-consistent with T1/T2/T4 but diagnostically fragile). \textsuperscript{b} Courts T5 row carries a low-power warning: 19 countries change over 2013--2022, between the validity threshold of ten and the low-power threshold of twenty-five. Reported for completeness with the caveat.}
\end{table}

Five patterns stand out. First, the composite is strongly and negatively associated with women's welfare in the cross-section: the 2020 coefficient is $-0.226$ ($p < 0.001$), and the 2014 value is similar at $-0.194$. Second, the composite's Mundlak between-country coefficient is $-0.278$ ($p < 0.001$), confirming that the structural cross-national gap between more and less secular countries is large and highly significant. Third, the composite's within-country coefficient over the 2013--2022 window is mildly positive rather than negative: $+0.061$ in both the TWFE and Mundlak within components, with $p \approx 0.07$ under entity-clustered standard errors. Under Driscoll--Kraay HAC standard errors the same point estimate tightens to $p < 0.001$, but the sign does not change. The between-to-within ratio (in magnitude) is $4.52$, and the two point in opposite directions. Fourth, the long-difference regression corroborates the within-country null at the medium-run horizon: the 2013--2022 decade-change coefficient on the composite is $+0.082$ ($p = 0.28$, $N = 163$, 158 changers), wrong-signed for the prior and not statistically distinguishable from zero. Fifth, the PCA variant of the composite produces the same cross-section and between-country pattern, with a smaller and less precisely estimated within-country coefficient ($+0.009$, $p = 0.69$), which is consistent with the PCA variant loading more heavily on the higher-coverage institutional dimension and therefore carrying less within-country variation.
...
\caption{Long-difference regression of the decade-change in the women's treatment index on the decade-change in each focal predictor, 2013--2022 with GDP. Left panel: forest plot of $\beta$ with $95\%$ confidence intervals across seven focal variants. Apostasy is greyed and flagged ``invalid'' because only nine countries record any apostasy-law movement over the window; courts is flagged ``low-power'' with 19 changers. Right panel: decade-change scatter for the headline equal-weight composite, one point per country, region-coloured, with OLS fit and $95\%$ confidence band. Source: \texttt{results/results.csv}, tier \texttt{T5\_long\_diff\_2013\_2022\_with\_gdp}.}
\label{fig:longdiff}
\end{figure}

Figure~\ref{fig:longdiff} reports the long-difference regression. The motivation is that the Tier 2 TWFE and Tier 4 Mundlak within coefficients are identified off year-to-year movement, a share of which is short-term noise or, for the composite's behavioural dimension, interpolator arithmetic between WVS waves. Long-differencing retains only endpoint observations, removes annual variation from the identifying signal, and tests whether a country that moved substantially on the composite over ten years also moved on the outcome in the direction the cross-sectional coefficient predicts. The canonical use of this design in comparative development is \citet{AcemogluJohnsonRobinsonYared2008} and \citet{BesleyPersson2011}.

The main 2013--2022 with-GDP result on the equal-weight composite is $\hat{\beta} = +0.082$ ($\mathrm{SE} = 0.076$, $p = 0.28$, $N = 163$, 158 changers): wrong-signed relative to the cross-sectional and between-country prior, small in absolute magnitude, and not statistically distinguishable from zero. The alternate 2014--2022 endpoint gives $+0.037$ ($p = 0.60$). The five-year sub-windows return $+0.162$ ($p = 0.06$) for 2013--2017 and $+0.107$ ($p = 0.12$) for 2017--2022. Both sub-window point estimates are larger in absolute magnitude than the full-decade estimate and both carry the same positive (wrong-signed-relative-to-prior) direction. The 2013--2017 window reaches marginal significance at the $10\%$ level, which deserves acknowledgement rather than being folded into the null: if a positive within-country signal were going to emerge over a medium-run horizon, the first half of the decade is where it would show up, and a $p = 0.06$ positive coefficient is the closest the paper comes to evidence against the structural reading. We read the sub-window pattern as consistent with the full-decade estimate (same direction, wrong-signed for the prior) rather than as independent corroboration of a negative effect, but the $p = 0.06$ point estimate is on the record and a reader should weigh it for themselves.

The PCA and real-wave-only composite variants collapse further toward zero under long-differencing: $\hat{\beta}_{\text{PCA}} = -0.015$ ($p = 0.79$) and $\hat{\beta}_{\text{real}} = +0.020$ ($p = 0.73$) on the 2013--2022 with-GDP window. Both variants are designed to reduce the interpolator share of the within-country signal, and both deliver coefficients closer to zero than the equal-weight composite does, which is consistent with the interpretation in \S\ref{sec:within-null} that the equal-weight composite's within-country estimate is partly interpolator-driven. Of the seven focals tested, apostasy is the only one with a sign-correct point estimate ($-0.042$, $p = 0.17$), but it is identified off only nine changer countries across the decade and is flagged invalid; the sign is direction-consistent with T1, T2 and T4 but the estimate is diagnostically fragile. The courts focal (legacy null) remains small and insignificant ($-0.010$, $p = 0.48$, low-power warning with 19 changers).

The long-difference evidence points the same direction as the short-panel within-country estimates. The within-country null at the annual scale is not an artefact of short-run noise or WVS interpolation: it persists when the estimator collapses the panel to endpoint changes and throws the interpolator-smoothed annual movement away. The structural cross-country reading of the paper therefore survives a medium-run within-country check rather than being rescued by it, and the honest statement is that the 2013--2022 data does not provide positive within-country evidence at either the annual or the decade horizon.

\section{Discussion}

The five tiers and the robustness battery produce three substantive findings.
...
To quantify how much of the within-country coefficient is interpolator-driven, we re-estimate every tier with the real-wave-only composite (\texttt{composite\_secularism\_real\_norm}) as the focal predictor. The real variant's within-country standard deviation on the $[0, 1]$ scale is $0.048$, slightly larger than the main composite's $0.039$: removing the smoothed interpolation introduces discrete jumps at wave boundaries rather than shrinking the within-country spread, so the coefficient comparison is not contaminated by a mechanical variance collapse. Cross-sectionally, the 2020 with-GDP coefficient shrinks from $-0.226$ to $-0.193$ (about $15\%$) and remains highly significant; the structural between-country gap survives the restriction. Within-country the coefficient falls from $+0.061$ ($p = 0.07$) to $+0.027$ ($p = 0.27$) under entity-clustered standard errors, and the wild-cluster bootstrap places the real variant's within-country $p$-value at $0.17$ (against $0.06$ for the main composite). The within-country null that the main composite shows is therefore largely preserved in the real variant, but the wrong-signed positive within-country point estimate is roughly halved, and part of its marginal significance was being carried by interpolator arithmetic rather than by real wave-observed movement. The Driscoll--Kraay standard error on the real variant's T2 within-country coefficient stays tight ($0.008$, giving $p < 0.001$ on a $+0.027$ point estimate), but on a $T = 10$ panel with bandwidth-2 Bartlett kernel this reflects the kernel's short memory rather than a genuine robust inference, and we treat the entity-clustered and wild-bootstrap numbers as the informative ones.

The long-difference regression in \S\ref{sec:longdiff} provides a complementary test that sidesteps annual noise and interpolator contamination by construction: it uses only the 2013 and 2022 endpoint observations, so WVS linear-interpolation at inter-wave years cannot contribute to the identifying signal. The composite long-difference coefficient is $+0.082$ ($p = 0.28$, $N = 163$), wrong-signed for the prior and not statistically distinguishable from zero. The PCA and real-wave-only variants collapse further toward zero under long-differencing ($-0.015$ and $+0.020$ respectively). The within-country null therefore persists at the medium-run horizon, which rules out the interpretation that the short-panel null is driven by annual noise or by the WVS-interpolation issue alone; if either of those had been the binding constraint, the long-difference design would have recovered a sign-correct estimate. The honest reading is that the 2013--2022 data do not support a within-country mechanism at any horizon they can identify, and the structural cross-country gap is the load-bearing finding.
