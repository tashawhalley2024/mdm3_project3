...
We also build and report \texttt{composite\_secularism\_pca\_norm} as a robustness variant. The PCA variant applies column-mean imputation to each of the eleven inputs, standardises the columns, and takes the first principal component of the resulting matrix; the loading on \texttt{gri\_state\_religion\_norm} is used to fix the sign. The first principal component explains about $37\%$ of the variance in the eleven inputs. Column-mean imputation reduces the post-fill variance of the lower-coverage WVS columns relative to the fully-covered institutional columns, which pushes the first PC toward a weighted institutional average; the PCA variant is therefore not an independent construction but a useful cross-check on the equal-weight composite.
